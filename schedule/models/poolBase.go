...
func (p *PoolBase) SavePoolBase(chainId, poolId string, poolBase *PoolBase) error {
	...
	//save token info
	symbol, err := p.SaveTokenInfo(poolBase)
	if err != nil {
...
func (p *PoolBase) SaveTokenInfo(base *PoolBase) ([]string, error) {
	tokenInfo := TokenInfo{}
	tokenSymbol := []string{"", ""}
	nowDateTime := utils.GetCurDateTimeFormat()
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			return tokenSymbol, errors.New("token_info record select err " + err.Error())
		}
	}
