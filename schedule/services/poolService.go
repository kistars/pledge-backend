...
func (s *poolService) UpdatePoolInfo(contractAddress, network, chainId string) {
	...
	for i := 0; i <= int(pLength.Int64())-1; i++ {
		...
		borrowToken, _ := models.NewTokenInfo().GetTokenInfo(baseInfo.BorrowToken.String(), chainId)
		lendToken, _ := models.NewTokenInfo().GetTokenInfo(baseInfo.LendToken.String(), chainId)
